...
def analyze_jobs():
    ...
    fields = ["Title", "Company", "Location", "Easy Apply", "Match Score",
              "Priority", "Matched Skills", "Missing Skills", "Link"]
    ...
    for job in jobs:
        ...
        if score < MIN_MATCH_SCORE or not location_allowed(job["Location"]):
            continue
        if EASY_APPLY_FILTER and job["Easy Apply"].strip().lower() != "yes":
            continue
    ...
    if recommended:
        for i, job in enumerate(recommended[:TOP_JOBS], 1):
            ...
            print(f"   Easy Apply: {job['Easy Apply'] or 'Unknown'}")
            print(f"   Missing : {job['Missing Skills'] or 'None'}")
    else:
        print(f"\nNo jobs currently meet the {MIN_MATCH_SCORE}% score and configured location filters.")

    print("\n" + "=" * 60)
    print("RECOMMENDATION SUMMARY")
    print("=" * 60)
    print(f"Minimum Match Score : {MIN_MATCH_SCORE}%")
    print(f"Easy Apply Filter   : {EASY_APPLY_FILTER}")
    ...
    if counter:
        ...
        for rank, (skill, count) in enumerate(counter.most_common(10), 1):
            print(f"{rank}. {skill} -> {count} job(s)")
        print("\nRecommended Focus:")
        print(", ".join(s for s, _ in counter.most_common(5)))
    else:
